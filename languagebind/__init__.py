...
from .video.processing_video import LanguageBindVideoProcessor, LanguageBindFrameIdsProcessor
...
class LanguageBind_Vid(nn.Module):
    def __init__(self, clip_type, use_temp=True):
        super(LanguageBind_Vid, self).__init__()
        self.use_temp = use_temp
        self.modality_encoder = {}
        self.modality_proj = {}
        self.modality_scale = {}
        self.modality_config = {}
        for k, v in clip_type.items():
            pretrained_ckpt = f'/media/qiuyc/data_disk/weights/LanguageBind/LanguageBind_Video_Huge_V1.5_FT'
            model = model_dict[k].from_pretrained(pretrained_ckpt, device_map="cuda")
            self.modality_encoder[k] = model.vision_model
            self.modality_proj[k] = model.visual_projection
            self.modality_scale[k] = model.logit_scale
            self.modality_config[k] = model.config
