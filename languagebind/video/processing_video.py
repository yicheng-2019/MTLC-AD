...
def load_and_transform_frameIds(
        video_path,
        transform,
    ...
    """
    该函数根据传入的视频帧索引加载并转换特定的视频帧。
    如果未指定帧索引，则会执行默认的均匀采样。

    Args:
    - video_path (str): 视频文件路径。
    - transform (callable): 用于转换视频帧的预处理函数。
    - video_decode_backend (str): 视频解码后端，可以是 'decord'。
    - clip_start_sec (float): 视频开始时间（秒）。
    - clip_end_sec (float): 视频结束时间（秒）。
    - num_frames (int): 要提取的帧数。
    - frame_id_list (list of int): 指定的帧索引列表，如果为None，则进行均匀采样。

    Returns:
    - torch.Tensor: 经过转换的视频帧。
    ...
    if video_decode_backend == 'decord':
        ...
        # 如果frame_id_list为None，进行默认的均匀采样
        ...
        raise NameError('video_decode_backend should specify in (decord)')
    return video_outputs
...
class LanguageBindFrameIdsProcessor(ProcessorMixin):
    attributes = []
    tokenizer_class = ("LanguageBindVideoTokenizer")

    def __init__(self, config, transform, tokenizer=None, **kwargs):
        super().__init__(**kwargs)
        self.config = config
        self.transform = transform
        self.image_processor = load_and_transform_frameIds
        self.tokenizer = tokenizer

    def __call__(self, video_path, frame_ids=None, text=None, context_length=77, return_tensors=None, **kwargs):
        # if text is None and images is None:
        # raise ValueError("You have to specify either text or images. Both cannot be none.")
        ...
        if frame_ids is not None:
            # frame_ids = make_list_of_images(frame_ids)
            ...
            ) for frame_id_list in frame_ids]

            image_features = torch.stack(image_features)

        if text is not None and frame_ids is not None:
            encoding["pixel_values"] = image_features
            return encoding
        elif text is not None:
            return encoding
